...
exports.Server = function Server(bsClient, workers, config, callback) {
  ...
  var handlers = {
    ...
    '_report': function reportHandler(uri, body, request, response) {
      ...
      var childSuiteStatus = query.childSuites.map(function(childSuite) {
        return childSuite.name + ": " + childSuite.status
      });
      if (reason !== '') {
        reason = reason + "; " + childSuiteStatus;
      } else {
        reason = childSuiteStatus;
      }
      
      if (testStatus === null || (testStatus !== query.status && testStatus !== 'failed')) {
        testStatus = query.status;
      }
      
      worker.setTestStatus(
        testStatus,
        reason
      );
